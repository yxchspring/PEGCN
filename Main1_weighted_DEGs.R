...
# set your path
setwd("your folder path of this procedure")
rm(list = ls())
# load the required packages
library(CancerSubtypes)

############## Step 1: Load data
### load the brca tumor data
load("ds1_brca_tumor.Rdata")
...
for (ii in subtypes_names) {
  ...
  ### get the results of DEGs using limma
  subtype_results=DiffExp.limma(Tumor_Data=brca_tumor_new[,-index_subtype], # control group
                                Normal_Data=brca_tumor_new[,index_subtype], # experiment group
...
for (kk in c(1:length(subtypes_names))) {
  subtype_weighted_DEGs_classification[[kk]] <- intersect(subtype_DEGs_filter_list[[kk]],DEGs_ranking_order$GeneName[c(1:2000)])
}
...
for (kk in c(1:length(subtypes_names))) {
  subtype_weighted_DEGs_pathway[[kk]] <- intersect(subtype_DEGs_filter_list[[kk]],DEGs_ranking_order$GeneName[c(1:4000)])
}
